...
#ifdef USE_SEESCORELIB
#import <SeeScoreLib/CIncludes.h>
#else
#include "sscore.h"
...
/*!
 @method hasUndo
 @abstract is there an undo state?
 @return true if is possible to undo the last edit
 */
-(bool)hasUndo;
...
/*!
 @method hasRedo
 @abstract is there a redo state?
 @return true if is possible to redo the last undo
 */
-(bool)hasRedo;
...
/*!
 ...
 so if the change affects only one bar then only that needs to be updated.
 All parts of the UI which show a representation of any part of the score should use a handler so
 everything updates automatically on a state change
...
/*!
 ...
 @param the id returned from addChangeHandler
 */
